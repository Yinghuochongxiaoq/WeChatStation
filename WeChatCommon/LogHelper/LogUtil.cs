...
namespace WeChatCommon.LogHelper
{
    /// <summary>
    /// 写日志类
    /// </summary>
    public class LogUtil
    {
        ...
        private static long _sizeInfoLog;
        private static long _sizeErrorLog;
        #endregion
        ...
        private static void WriteFile(string log, string path, ref long fileSize)
        {
            try
            {
                ...
                {
                    Directory.CreateDirectory(Path.GetDirectoryName(path) ?? throw new InvalidOperationException("path 参数不能为空"));
                }
